...
# Load YOLOv8 based on YOLOv5 framework
model = torch.hub.load('ultralytics/yolov5', 'custom', path='F:\Works\Freelancing\JNU_Project\YOLOv8-Real-Time-Drone-Detection-by-Sabbir/best.pt', source='github')
...
while True:
    ...
    # Convert the frame to a format that YOLOv8 based on YOLOv5 framework can process
        ...
        if conf > 0.5 and classes[int(cls)] in classes:
            ...
            if rectangle_coords[0] != rectangle_coords[1]:
                if any(rectangle_coords[0][0] <= x <= rectangle_coords[2][0] and rectangle_coords[0][1] <= y <= rectangle_coords[2][1] for x, y in
                    ...
                    cv2.putText(frame, "YOLOv8: Drone Detected Under Restricted Area!", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
